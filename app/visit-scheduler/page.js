...
export default function MyProperty() {
  ...
  const fetchData = async (page = 1) => {
    try {
        ...
        const getUserInfo = await insertData('api/visit/get-user-visit-schedule', requestData, true); // Ensure data is passed
                            ...
                            <tbody>
                              {propertiesVisits.map(user => (
                                <tr key={user.id} className="file-delete">
                                  ...
                                  <td>
                                    <span>{user.property.users.email_address}</span>
                                  </td>
